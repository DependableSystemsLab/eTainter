...
from src.util.utils import is_subseq, is_substr
    ...
    def find(self, slices, looplimit=2, avoid=frozenset(), prefix=None):    
        ...
        while not todo.empty():                         
            ...
            if any(is_substr(pth, state.path) for pth in self.blacklist):
                logging.debug("BLACKLIST hit for %s" % (', '.join('%x' % i for i in state.path)))
                continue            
            ...
            if state.finished:
                ...
                if not state.slices:                    
                    continue
            if state.path.count(state.bb.start) > vLooplimit:                                     
                c+=1                
                if c>5:  #In case where a function contains several loops, finishing a path takes very long-time, to avoid we reduce looplimit to 1 
                    vLooplimit=1 
                continue
