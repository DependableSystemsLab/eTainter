...
from src.evm.state import AbstractEVMState
...
class taint(object):
    ...
    def isTainted_register(self, reg, from_storage=False):        
        ...
        if not from_storage:            
            ...
            if (reg._writer._return_value in [regs[0] for regs in self.tRegisters if regs[1]!={} and any(regs[1].startswith('SLOAD'))]):
                return True
            else:            
                return False            
    ...
    while state.pc in program:        
        ...
        if ((inclusive and len(path) == 0)
            ...
            if state.pc in sinks.keys():                                         
                if defect_type in set(['Storage-Access']):                                                                                
                    if ssa_ins[0].arguments[0]._writer is None:
                        ...
                        if tnt.registers[ssa_ins[0].arguments[0]] in storage_slots or (set(tnt.get_reg_sha3_bases(ssa_ins[0].arguments[0])) & set(storage_slots+list(storage_sha3_bases.values()))):                                                    
                            tainted=True                        
                ...
                elif defect_type in (['Storage-Temp-Variables']): 
                    if ssa_ins[0].arguments[0]._writer is None:                                        
                        sstore_slots.append(ssa_ins[0].arguments[0].concrete_value)
                    else:                        
                        sstore_slots.append(tnt.registers[ssa_ins[0].arguments[0]])                                                                  
                        loc_bases=tnt.get_reg_sha3_bases(ssa_ins[0].arguments[0])                                                
                        loc_type=tnt.get_reg_slot_type(ssa_ins[0].arguments[0])
                        
                        if len(loc_bases)!=0:
                            sstore_bases[tnt.registers[ssa_ins[0].arguments[0]]]=loc_bases[0]                                                                       
                        if len(loc_type)!=0:
                            storage_slot_type[tnt.registers[ssa_ins[0].arguments[0]]]=loc_type[0]
                    tainted=True                                   
                elif defect_type in (['Hardcoded-Gas']):              
                    for i in sinks[state.pc]:                                            
                        if (ssa_ins[0].arguments[0]._writer is not None and ssa_ins[0].arguments[0]._writer.insn.name in('GAS')) \
                        or (ssa_ins[0].arguments[2]._writer is None and ssa_ins[0].arguments[2].concrete_value ==0) :                                                        
                            tainted=False
                            continue
                        else:                            
                            tainted=True
                            #print(ssa_ins[0].arguments[2].concrete_value)                
                            if ssa_ins[0].arguments[i].writer is not None:
                                sources=tnt.get_reg_taint_source(ssa_ins[0].arguments[i])              

                elif defect_type in (['Gas-Griefing']):        
                    for i in sinks[state.pc]:     
                        if ssa_ins[0].arguments[i].writer is not None:
                            loc =tnt.registers[ssa_ins[0].arguments[i]]
                        else:
                            loc =ssa_ins[0].arguments[i+1].concrete_value                        

                        if ssa_ins[0].arguments[i+1].writer is not None:
                            loc_size =tnt.registers[ssa_ins[0].arguments[i+1]]
                        else:
                            loc_size =ssa_ins[0].arguments[i+1].concrete_value
                                                
                        if tnt.isTainted_memory(loc,loc_size):                                                                    
                            sources=tnt.get_mem_taint_source(loc, loc_size)                                                                                                                                                              
                            taint_origin= [k for i in sources for k, v in i.items() if k in (['CALLDATACOPY','CALLDATALOAD'])]                                                                           
                            if len(taint_origin)!=0:
                                tainted=True                            
                                break        
                else:                    
                    for i in sinks[state.pc]:     
        ...
        # if not, have we reached another step of our path
        elif state.pc == path[0]:  #jumped to the next block in the path             
            ##get_ssa_block
            ssa_block=[ins for block in function if block.offset ==path[0] for ins in block.insns]                                                        
            #ssa_block_in_edges= [blk.offset for block in function if block.offset ==path[0] for blk in block.in_edges]                        
            ...
            path = path[1:]                
            #Do this if you updated JUMP 
            #if we jumped to a block not the next block in the path
        ...
        elif program[state.pc].bb.start==state.pc and state.pc not in[b.offset for b in ssa_block] and state.pc not in path:                        
            state.success = False                
            return TainitAnalysisResult(state,defect_type,target_sink,tainted,sources,sload_bases,sstore_bases,sstore_slots,slot_live_access,slot_access_trace, storage_slot_type)                                                
        ...
        if (op == 'JUMPDEST' and len(ssa_ins)!=0) or (ins.addr==ins.bb.start and len(ssa_ins)!=0 and str(ssa_ins[0].insn)=='PHI'):
                ...
                for insn in ssa_ins:                                                              
                    if len(insn.arguments)!=0:
                            ...
                            if ssa_last_visited > last_visited:
                                if insn.arguments[i]._writer is not None and insn.arguments[i].writer.parent_block.offset ==ssa_last_visited:                                                                                                    
                                    if insn.arguments[i] in tnt.registers:
                                        ...
                                        #Dpeneding on prdecessor ins not visited in the current path       
                        ...
                        tnt.registers[insn._return_value] =55 ##Fix rattle code to check why PHI with no parameters 
        ...
        # Arithmetic
            ...
            #Infeasiblepath                        
                ...
                if ssa_ins[0].arguments[1]._writer is None:                        
                    ...
                    #Consider the cases where storage address is not calculated using SHA3 and is provided as const in the ADD
               ...
               pass               
                ...
                if len(ssa_block)!=0:
                    ...
                    if state.pc not in original_path:                        
                        #Destination block not in the current path                        
                        state.success = False
                        return TainitAnalysisResult(state,defect_type,target_sink,tainted,sources,sload_bases,sstore_bases,sstore_slots,slot_live_access,slot_access_trace, storage_slot_type)            
        ...
        elif op == 'REVERT':  #No need to implement its effects on memory as it will stop execution
            return TainitAnalysisResult(state,defect_type,target_sink,tainted,sources,sload_bases,sstore_bases,sstore_slots,slot_live_access,slot_access_trace, storage_slot_type)            
