...
from src.evm.exceptions import ExternalData, SymbolicError, IntractablePath, VMException
from src.evm.results import SymbolicResult, gen_exec_id
from src.evm.state import SymRead, EVMState, SymbolicEVMState
from src.util.z3_extra_util import concrete, is_true
    ...
    while state.pc in program:
        ...
        # Arithmetic
            ...
            if op == 'STOP':
                ...
                s0, s1 = src.util.utils.to_signed(stk.pop()), src.util.utils.to_signed(stk.pop())
                ...
                stk.append(pow(base, exponent, src.util.utils.TT256))
            elif op == 'SIGNEXTEND':
                s0, s1 = stk.pop(), stk.pop()
                if s0 <= 31:
                    testbit = s0 * 8 + 7
                    if s1 & (1 << testbit):
                        stk.append(s1 | (src.util.utils.TT256 - (1 << testbit)))
                    else:
        ...
        # Comparisons
            ...
            if op == 'LT':
                ...
                stk.append(src.util.utils.TT256M1 - stk.pop())
            elif op == 'BYTE':
                s0, s1 = stk.pop(), stk.pop()
                if s0 >= 32:
                    stk.append(0)
                ...
                s0, s1 = stk.pop(), src.util.utils.to_signed(stk.pop())
                stk.append((s1 >> s0))
        # SHA3 and environment info
            ...
            if op == 'SHA3':
                s0, s1 = stk.pop(), stk.pop()
                mem.extend(s0, s1)
                data = src.util.utils.bytearray_to_bytestr(mem[s0: s0 + s1])
                stk.append(src.util.utils.big_endian_to_int(src.util.utils.sha3(data)))
            elif op == 'ADDRESS':
                ...
                stk.append(src.util.utils.bytearray_to_int(ctx.calldata[s0:s0 + 32]))
        ...
        # VM state manipulations
            ...
            if op == 'POP':
                ...
                s0 = stk.pop()
                mem.extend(s0, 32)
                stk.append(src.util.utils.bytes_to_int(mem[s0: s0 + 32]))
            elif op == 'MSTORE':
                s0, s1 = stk.pop(), stk.pop()
                mem.extend(s0, 32)
                mem[s0: s0 + 32] = src.util.utils.encode_int32(s1)
            elif op == 'MSTORE8':
                s0, s1 = stk.pop(), stk.pop()
                mem.extend(s0, 1)
                mem[s0] = s1 % 256
        ...
        # Arithmetic
            ...
            if op == 'STOP':
                ...
                if concrete(s0) and concrete(s1):
                    s0, s1 = src.util.utils.to_signed(s0), src.util.utils.to_signed(s1)
                    stk.append(0 if s1 == 0 else abs(s0) // abs(s1) *
                                                 (-1 if s0 * s1 < 0 else 1))
                elif concrete(s1):
                    stk.append(0 if s1 == 0 else s0 / s1)
                ...
                if concrete(base) and concrete(exponent):
                    ...
                    if concrete(base) and src.util.utils.is_pow2(base):
                        l2 = src.util.utils.log2(base)
        ...
        # SHA3 and environment info
            ...
            if op == 'SHA3':
                ...
                if not isinstance(mm, SymRead) and all(concrete(m) for m in mm):
                    data = src.util.utils.bytearray_to_bytestr(mm)
                    stk.append(src.util.utils.big_endian_to_int(src.util.utils.sha3(data)))
                else:
        ...
        # VM state manipulations
            ...
            if op == 'POP':
                ...
                if all(concrete(m) for m in mm):
                    stk.append(src.util.utils.bytes_to_int(mem.read(s0, 32)))
                else:
                ...
                mem.extend(s0, 32)
                if concrete(s1):
                    mem.write(s0, 32, src.util.utils.encode_int32(s1))
                else:
                    for i in range(32):
